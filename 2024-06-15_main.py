...
#2024-06-16  версия 3.0(back-end) теперь надо доделать в plotly и  exe
import pandas as pd
...
def search_for_parent(child_dic, parent_dic):
    ...
    if '' in child_dic.values() or '' in parent_dic.values():# в Локусе есть ''
        if  all(value == '' for value in child_dic.values()) or all(value == '' for value in parent_dic.values()):# у одного в локусе обе буквы ==""
            no_data_loc_point = 1
        else:
            dif_no_ful_loc_point = 1
    for child_let in parent_dic.values():
        if child_let != '':
            char_count_df += 1
...
df = pd.read_csv('D:\\work\\23-12-11_STR_profiles_PyCharm\\2024-06-10_str_prof_github.csv'
...
child_name = '1_Name1'  # '2150_Румянец'#'2610_Танго РВ'#!!!! ВВОД Номера и Имени ЛОШАДИ в поиске
...
for parent_name in df['id_f']:# проход по всем строкам dataFrame
    ...
    num_loc_gl = 1 # вводим значение порога допустимого совпадения
