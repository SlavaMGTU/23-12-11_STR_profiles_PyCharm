...
# Если нажата кнопка "Определить_родство" напечатать "answ_fin" под кнопкой
...
def search_for_parents( child_row, parent1, parent2, numb_col, df):
...
def search_for_parents_all(child_row_gl, parent1_gl, parent2_gl, df):# MARKBOOK!!!! ЗАКЛАДКА
    ...
    for numb_col_gl in range(2, 36, 2):
        answ = search_for_parents(child_row_gl, parent1_gl, parent2_gl, numb_col_gl, df)
...
def update_output(list_of_contents, list_of_names):
    if list_of_contents is not None:
        ...
        # Возвращаем опции для всех трех выпадающих списков
        return df

# Обратная связь для обновления результатов по нажатию кнопки и напечатать результат под кнопкой
@app.callback(
...
def update_result(n_clicks, child_value, parent1_value, parent2_value, df):
    if n_clicks is not None and n_clicks > 0:
        # Вызов функции для определения родственных связей
        result = search_for_parents_all(child_value, parent1_value, parent2_value, df)
        return result
