...
import pandas as pd
import re
def det_of_relation_1Horse_app( child_name_gl, parent1_gl, parent2_gl, df):
    def search_for_parent(child_dic, parent_dic):
        ...
        if parent_point != 1:
            if '' in child_dic.values() or '' in parent_dic.values():  # в Локусе есть ''
                if len(set(child_dic.values())) == '' or len(
                        set(parent_dic.values())) == '':  # у одного в локусе обе буквы ==""
                    no_data_loc_point = 1
                else:
    ...
    for parent_name in df['id_f']:  # проход по всем строкам dataFrame
        ...
        if child_name != parent_name:  # обходим чтоб не сравнивать лошадь с самой собой
            ...
            for numb_col_gl in range(2, 36, 2):
                ...
                parent_point_new, dif_no_ful_loc_point_new, no_data_loc_point_new = search_for_parent(child_dic,
                                                                                                      parent_dic)
        ...
        index_relativ = 15  # вводим значение порога допустимого совпадения
        if parent_index_gl >= index_relativ:
            df_relativ.loc[len(df_relativ.index)] = [parent_name, parent_index_gl,
            ...
            answ_fin += (f"У Лошади: {child_name} совпадений {parent_index_gl}/17 с {parent_name} ; ")

            if dif_no_ful_loc_point_gl != 0:
                answ_fin +=(f' неполных общих проф: {dif_no_ful_loc_point_gl} Локус: {dif_no_ful_loc_name_gl} ; ')

            if no_data_loc_point_gl != 0:
                answ_fin += (f' НЕТ данных {no_data_loc_point_gl} Локус: {no_data_loc_name_gl}; ')
