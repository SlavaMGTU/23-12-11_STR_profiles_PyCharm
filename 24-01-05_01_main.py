...
def search_for_parents(child_row, parent1, parent2, numb_col):
    ...
    return answ
df = pd.read_csv('D:\\work\\23-12-11_STR_profiles_PyCharm\\STR профили для программы1.csv'
...
#данные по поиску
child_row_gl, parent1_gl, parent2_gl = 196,197,198#198
...
if  parent1_index_gl >= 17:
    answ_fin += f'родитель1 - ДА ({parent1_index_gl} из 17) ;'
else:
    answ_fin += f'родитель1 - НЕТ ({parent1_index_gl} из 17) ;'

if  parent2_index_gl >= 17:
    answ_fin += f'родитель2 - ДА ({parent2_index_gl} из 17) ;'
else:
    answ_fin += f'родитель2 - НЕТ ({parent2_index_gl} из 17) ;'
