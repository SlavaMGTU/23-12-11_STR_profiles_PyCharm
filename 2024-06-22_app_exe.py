    ...
    if n_clicks > 0:
        if contents is not None:
            ...
            if df is None:
                ...
                row = df_table[df_table['Имя'] == child_value]
                f_string = (f"Имя: {row['Имя'].iloc[0]}, Букв в Проф: {row['Букв в Проф'].iloc[0]},"
                            f" Неполн Лок-ов: {row['Кол-во Неполн Лок-ов'].iloc[0]} - {row['Назв. Неполн Лок-ов'].iloc[0]},"
                            f" Нет Лок-ов: {row['Нет Лок-ов Кол-во'].iloc[0]} - {row['Нет Лок-ов Имя'].iloc[0]}")
...
#pyinstaller --onedir 2024-06-20_app_exe.py
