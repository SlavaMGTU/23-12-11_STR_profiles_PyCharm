import base64
import io
import webbrowser
from threading import Timer

#рабочая программа: PLOTLY+PyCHarm; скачивает данные из csv файла; выбирает лошадь вo всплывающем поле ; выбирает
# в поле количество совпадающих локусов; нажимает кнопку провести сравнение ; печатает интерактивную таблицу рез-тов
# Поиск родственников для ЗАДАННОй Лошади
#ИСПРАВЛЕНИЕ: в таблицы неполные данные и нет данных только для той самой лошади

from dash import Dash, dash_table, dcc, html
import pandas as pd
from dash.dependencies import Input, Output, State

from determ_of_relation_1Horse2024_09_21 import det_of_relation_1Horse_app

external_stylesheets = ['https://codepen.io/chriddyp/pen/bWLwgP.css']

# table_data = {'ID': [],
#                 'Имя': [],
#                 'Локусов совпало': [],
#                 'Кол-во Неполн Лок-ов': [],
#                 'Назв. Неполн Лок-ов': [],
#                 'Нет Лок-ов Кол-во': [],
#                 'Нет Лок-ов Имя': [],
#                 'Букв в Проф': [],
#                 }
#
# df_table = pd.DataFrame(table_data)

app = Dash(__name__, external_stylesheets=external_stylesheets)
app.layout = html.Div([
    dcc.Upload(
        id='upload-data',
        children=html.Div([
            'Drag and Drop or ',
            html.A('Select Files')
        ]),
        style={
            'width': '100%',
            'height': '60px',
            'lineHeight': '60px',
            'borderWidth': '1px',
            'borderStyle': 'dashed',
            'borderRadius': '5px',
            'textAlign': 'center',
            'margin': '10px'
        },
    ),
    html.Div(id='output-data-upload'),# !!!!результат загрузки данных из файла

    # html.Label('Сравниваемая'),
    # dcc.Dropdown(
    #     id='child_dropdown',
    #     options=[],
    #     value=None,
    #     placeholder="Select Horse"
    # ),

    html.Div([
        html.Label('Сравниваемая', style={'flex': '0'}),
        dcc.Dropdown(
            id='child_dropdown',
            options=[],
            value=None,
            placeholder="Select Horse",
            style={'flex': '1'}
        ),
    ], style={'display': 'flex'}),

    html.Div([
        html.Button('Поиск возможных родственников', id='relation_button', n_clicks=0),  # Добавляем кнопку "Определение родства"

        dcc.Input(
            id="num_loc", type="number", value= 15,
            debounce=True, placeholder="Debounce True",
        ),

        html.Div(  # Добавляем элемент с текстом - подписью/рекламой
            """https://github.com/SlavaMGTU/23-12-11_STR_profiles_PyCharm\nSlavaMGTU@gmail.com""",
            style={'fontSize': '8px', 'color': 'gray', 'white-space': 'pre-line'}
        )

    ], style={'display': 'flex'}),

    # html.Div(id='output'),  # Для вывода результата после нажатия кнопки
    html.Div(id='table_fin')  # Блок для отображения таблицы с датафреймом table_data
])

def open_browser():
	webbrowser.open_new("http://localhost:{}".format(8050))

def parse_data(contents, filename):
    content_type, content_string = contents.split(',')

    decoded = base64.b64decode(content_string)
    try:
        if 'csv' in filename:
            # Assume that the user uploaded a CSV or TXT file
            df = pd.read_csv(io.StringIO(decoded.decode('utf-8')))
        elif 'xls' in filename:
            # Assume that the user uploaded an excel file
            df = pd.read_excel(io.BytesIO(decoded))
        elif 'txt' or 'tsv' in filename:
            # Assume that the user upl, delimiter = r'\s+'oaded an excel file
            df = pd.read_csv(
                io.StringIO(decoded.decode('utf-8')), delimiter = r'\s+')
        if df.shape[1] == 1:  # Проверка, что в DataFrame только один столбец
            # Разделение заголовков столбцов
            column_names = df.columns[0].split(';')

            # Разделение данных на строки и столбцы
            data_rows = df.iloc[:, 0].str.split(';', expand=True)

            # Создание нового датафрейма с правильными именами столбцов
            df = pd.DataFrame(data_rows.values, columns=column_names)
    except Exception as e:
        print(e)
        return html.Div(['There was an error processing this file.'])

    return df

@app.callback(
    Output('child_dropdown', 'options'),
    [Input('upload-data', 'contents'),
     Input('upload-data', 'filename')])
def update_child(contents, filename):
    if contents:
        df = parse_data(contents, filename)
        df = df.set_index(df.columns[0])
        lst = [{'label': i, 'value': i} for i in df['кличка'].unique()]
        return lst
    else:
        return []

@app.callback(
    Output('table_fin', 'children'),  # Используем блок table_fin для отображения таблицы
    [Input('relation_button', 'n_clicks'),
     Input('upload-data', 'contents'),
     Input('upload-data', 'filename')],
    [State('child_dropdown', 'value'),
     State('num_loc', 'value')],
)
def update_output(n_clicks, contents, filename, child_value, num_loc):
    if n_clicks > 0:
        if contents is not None:
            df = parse_data(contents, filename)

            if df is None:
                return html.Div(['There was an error processing this file.'])
            else:
                df_table = det_of_relation_1Horse_app(child_value, df, num_loc)

                # Найти строку с child_value
                row = df_table[df_table['Кличка'] == child_value]
                f_string = (f"Имя: {row['Кличка'].iloc[0]}, Букв в профиле: {row['Букв в профиле'].iloc[0]},"
                            f" Локусы с 1 буквой: {row['Число лок. с 1 буквой'].iloc[0]} - {row['Локусы с 1 буквой'].iloc[0]},"
                            f" Пустые локусы: {row['Число пустых лок.'].iloc[0]} - {row['Пустые локусы'].iloc[0]},"
                            f" Строк найдено: {df_table.shape[0]-1}")

                # Удаление строки
                df_table = df_table[df_table['Кличка'] != child_value]


                return f_string, dash_table.DataTable(
                    id='table',
                    columns=[{'name': i, 'id': i} for i in df_table.columns],
                    data=df_table.to_dict('records'),
                    sort_action='native',  # Разрешение на сортировку в браузере
                    #page_size=10,
                    style_table = {
                        'overflowX': 'auto',  # Добавить горизонтальную прокрутку для таблицы
                        'overflowY': 'auto',
                        'height': '400px',
                        'width': '100%',
                        'margin-left': 'auto',
                        'margin-right': 'auto',
                    },
                    style_cell = {
                        'textAlign': 'left',
                        'whiteSpace': 'normal',
                        'height': 'auto'
                    },
                    style_header={
                        'textAlign': 'left',
                        'whiteSpace': 'normal',
                        'height': 'auto',
                        # Закрепить заголовок
                        'position': 'sticky',
                        'top': 0,
                        'background-color': 'white',
                        'z-index': 1
                    },
                )


    else:
        return ''


if __name__ == '__main__':
    Timer(1, open_browser).start();
    app.run_server(debug=False, port=8050)
#pyinstaller --onedir 2024-09-21_app_exe.py
# переименовать в find_1-horse.exe
# cd /D D:\work\23-12-11_STR_profiles_PyCharm\dist\2024-05-13exe\2024-05-13exe.exe
#2024-05-13exe.exe
